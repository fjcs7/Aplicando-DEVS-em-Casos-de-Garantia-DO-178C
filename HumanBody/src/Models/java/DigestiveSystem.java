...
public class DigestiveSystem extends CoupledModelImpl implements StateVariableBased{ 
	private static final long serialVersionUID = 1L;
	protected SimulationOptionsImpl options = new SimulationOptionsImpl();
	
		public final Port<? extends Serializable> outSatiation= addOutputPort("outSatiation",Serializable.class);
		public final Port<? extends Serializable> outNutrients= addOutputPort("outNutrients",Serializable.class);
	...
	public DigestiveSystem(String nm) {
		super(nm);
		make();
	}
	public void make(){
		...
		addChildModel(Intestine);
		Mouth Mouth = new Mouth();
		addChildModel(Mouth);
		Stomach Stomach = new Stomach();
		addChildModel(Stomach);
		addCoupling(Stomach.outSatiation,this.outSatiation);
		addCoupling(Mouth.outFood,Stomach.inFood);
		addCoupling(Intestine.outNutrients,this.outNutrients);
		addCoupling(Stomach.outFood,Intestine.inFood);
		addCoupling(this.inFood,Mouth.inFood);
	...
	public static void main(String[] args){
		...
		DigestiveSystem model = new DigestiveSystem();
		model.options = options;
		if(options.isDisableViewer()){ // Command Line output only
			Simulation sim = new com.ms4systems.devs.core.simulation.impl.SimulationImpl("DigestiveSystem Simulation",model,options);
